<template>
  <div v-if="totalPages > 0"  class="vue-ads-flex vue-ads-m-2 vue-ads-px-0 vue-ads-text-xs">
    <slot :start="start + 1" :end="end" :total="totalItems">
      <div class="vue-ads-pr-2 vue-ads-leading-loose">
        {{ start + 1 }} - {{ end }} của {{ totalItems }} items
      </div>
    </slot>
    <div
      v-if="totalPages > 1"
...
export default {
  ...
  methods: {
    ...
    html(page, key) {
      if (key === 0) {
        return '<i class="ion-chevron-left"></i>';
      }

      if (key === this.pages.length - 1) {
        return '<i class="ion-chevron-right"></i>';
      }
